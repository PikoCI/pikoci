...
- **Worker tokens could reach every route not explicitly denied**: a JWT with `is_from_worker` skipped authorization outside a small secret-management deny-list, so the global worker token — printed to the server log at startup, never expiring — could create, update and delete pipeline configs. Worker access is now an allowlist of the routes a worker actually needs, closed by default to anything new; a team-scoped token is refused outside its team on every route, not only secret values. If you were using a worker token as a general API credential, switch to a user JWT or an API token ([#704](https://github.com/PikoCI/pikoci/issues/704)).
