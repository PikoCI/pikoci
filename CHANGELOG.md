...
- **Concurrency fixes in gRPC worker and HTTP server**: `ready_check` sleep now exits promptly on build cancellation (uses `select` with `ctx.Done()` instead of `time.Sleep`); API token last-used DB writes are bounded to 8 concurrent goroutines with excess silently dropped; the gRPC server main loop reacts to send errors immediately without waiting for the next recv; the unused `grpcStream` worker field and its mutex are removed ([#658](https://github.com/PikoCI/pikoci/issues/658)).
