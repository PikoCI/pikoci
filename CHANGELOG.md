...
- `for_each` and `matrix` on jobs: generate multiple job instances from a single definition using `for_each = toset(...)`, `for_each = { key = "value" }`, or `matrix { axis = [...] }`. Each instance gets independent builds, status, and logs. Downstream `passed` constraints automatically fan-in across all instances ([#193](https://github.com/PikoCI/pikoci/issues/193), [#472](https://github.com/PikoCI/pikoci/issues/472))
- HCL functions expanded to match Terraform: `startswith`, `endswith`, `strcontains`, `strlen`, `strrev`, `base64encode`, `base64decode`, `urlencode`, `timestamp`, `formatdate`, `timeadd`, `alltrue`, `anytrue`, `one`, `sum`, `transpose`, `toset`, `setproduct`, `setintersection`, `setunion`, `setsubtract`, `setsymmetricdifference`, `tostring`, `tonumber`, `tobool`, `tolist`, `tomap`, `chunklist`, `coalesce`, `coalescelist`, `compact`, `element`, `range`, `slice`, `zipmap`, `parseint`, `pow`, `log`, `signum`
