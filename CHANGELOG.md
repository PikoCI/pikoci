...
- **Static asset caching**: JS, CSS and images now serve from a per-build path (`/assets/<commit>/`, one-year immutable cache); the page itself is `no-cache`. Fixes Safari running a previous build's cached JS after a deploy. Docker images and `deploy.sh` don't pass the commit through yet, so those key off restart time instead ([#699](https://github.com/PikoCI/pikoci/issues/699)).
...
- **Unpinned `get` pulled the oldest version**: since lazy loading listed resource versions newest-first, `buildPullParams` — still reversing the list from when it was oldest-first — took the *oldest* version for any `get` without a pinned or `passed` version. A job with two repositories checked out a months-old commit of whichever one did not trigger the build. The newest version is now chosen by id, whatever order the list arrives in ([#694](https://github.com/PikoCI/pikoci/issues/694)).
...
- **Zombie processes accumulate as PID 1**: `pikoci` ran as PID 1 and only reaped its own children, leaving orphaned `git`/`curl` helper processes as permanent zombies until the container's pids cgroup limit was hit and `fork()` started failing. `tini` is now the entrypoint's init, reaping orphans while `pikoci` keeps handling its own children ([#681](https://github.com/PikoCI/pikoci/issues/681)).
