...
- **Configuration and secret store**: PikoCI can store configuration itself, with no external vault or file on the worker. Secrets are encrypted at rest with an age keypair and masked in build logs; plain values are stored as-is and shown in the clear. Team- and pipeline-scoped with pipeline overriding team, managed via `pikoci client config` or the API, and referenced from a pipeline with `secret "pikoci" { key = "..." }`. Entries are stored as secrets by default; `--plain` opts out. Encryption is optional: without `PIKOCI_SECRET_KEY` the server runs as before and plain values still work ([#667](https://github.com/PikoCI/pikoci/issues/667)).
