...
| `--external-url` | | | no | Public URL users reach the server at (e.g., `https://ci.example.com`). Required for OAuth/OIDC: it is the base of the `/auth/oauth/<provider>/callback` redirect URI, and OAuth logins fail with a configuration error until it is set. |
