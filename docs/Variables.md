...
The `secret` block label is the name of a `secret_type` defined in the pipeline, or `pikoci` for a value held in PikoCI's own [configuration store](Secrets.md). The block has two fields:
