...
func TestGetPipelineImage_HidesUnlinkedResources(t *testing.T) {
	...
	s.Builds.EXPECT().LatestBuildStatusByPipeline(ctx, "main", "my-pipeline").Return(map[string][]*build.Build{}, nil)
...
func TestGetPipelineImage_QuotesHyphenatedName(t *testing.T) {
	...
	s.Builds.EXPECT().LatestBuildStatusByPipeline(ctx, "main", "hello-world").Return(map[string][]*build.Build{}, nil)
...
func TestGetPipelineImage_PassedReusesPutOutputNode(t *testing.T) {
	...
	s.Builds.EXPECT().LatestBuildStatusByPipeline(ctx, "main", "artifact-pipeline").Return(map[string][]*build.Build{}, nil)
...
func TestGetPipelineImage_JobStatusColors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			s.Builds.EXPECT().LatestBuildStatusByPipeline(ctx, "main", "p").Return(map[string][]*build.Build{"j": tt.builds}, nil)
...
`)
	...
	s.Builds.EXPECT().LatestBuildStatusByPipeline(ctx, "main", "pikoci").Return(map[string][]*build.Build{}, nil)
...
func TestGetPipelineImage_WithBuildStatuses(t *testing.T) {
	...
	s.Pipelines.EXPECT().Find(ctx, "main", "my-pipeline").Return(pp, nil)
	s.Builds.EXPECT().LatestBuildStatusByPipeline(ctx, "main", "my-pipeline").Return(map[string][]*build.Build{
...
func TestGetPipelineImage_HideIntermediates(t *testing.T) {
	ctrl := gomock.NewController(t)
	s := newService(ctrl)
	ctx := context.TODO()
	...
	pp := &pipeline.Pipeline{
		...
		Jobs: []job.Job{
			...
			},
			{
				Name: "deploy",
				Plan: []job.PlanStep{
					{Type: job.StepTypeGet, Get: &job.GetStep{Type: "artifact", Name: "output", Trigger: true, Passed: []string{"build"}}},
				},
			},
		},
		Resources: []resource.Resource{
			{ID: 1, Canonical: "cron.timer", Name: "timer", Type: "cron"},
			{ID: 2, Canonical: "artifact.output", Name: "output", Type: "artifact"},
		},
	}
	...
	dot := string(img)
	// Entry-point resource should be kept
	assert.Contains(t, dot, `"cron.timer"`)
	// Direct job-to-job edge should exist
	assert.Contains(t, dot, `"build"--"deploy"`)
	// Put output node should NOT exist
	assert.NotContains(t, dot, `"build-artifact.output-out"`)
	// No cds shape nodes for intermediate resources
	lines := strings.Split(dot, "\n")
	for _, line := range lines {
		trimmed := strings.TrimSpace(line)
		if strings.Contains(trimmed, "->") || strings.Contains(trimmed, `"cron.timer"`) {
			continue
		}
		if strings.Contains(trimmed, `"artifact.output"`) && strings.Contains(trimmed, "cds") {
			t.Errorf("intermediate resource should not appear when hideIntermediates=true: %s", trimmed)
		}
	}
}

func TestGetPipelineImage_GroupParallel(t *testing.T) {
	ctrl := gomock.NewController(t)
	s := newService(ctrl)
	ctx := context.TODO()
	...
	pp := &pipeline.Pipeline{
		...
		Jobs: []job.Job{
			...
			},
			{
				Name: "lint",
				Plan: []job.PlanStep{
					{Type: job.StepTypeGet, Get: &job.GetStep{Type: "cron", Name: "timer", Trigger: true, Passed: []string{"build"}}},
				},
			},
			{
				Name: "vet",
	...
	dot := string(img)
	// Should contain the HTML table for the parallel group
	assert.Contains(t, dot, `PARALLEL (2 JOBS)`)
	assert.Contains(t, dot, "lint")
	assert.Contains(t, dot, "vet")
	// Should use plaintext shape for the group node
	assert.Contains(t, dot, "plaintext")
	// Individual job nodes for lint and vet should NOT appear as standalone rectangle nodes
	assert.NotContains(t, dot, `"lint" [`)
	assert.NotContains(t, dot, `"vet" [`)
	// The build job should still appear as a normal node
	assert.Contains(t, dot, `"build" [`)
}

func TestGetPipelineImage_GroupParallel_SingleJob(t *testing.T) {
	ctrl := gomock.NewController(t)
	s := newService(ctrl)
	ctx := context.TODO()
	...
	dot := string(img)
	// Single child should NOT be grouped
	assert.NotContains(t, dot, "PARALLEL")
	// Deploy should appear as a normal rectangle node
	assert.Contains(t, dot, `"deploy" [`)
}

func TestGetPipelineImage_GroupParallel_WithHideIntermediates(t *testing.T) {
	ctrl := gomock.NewController(t)
	s := newService(ctrl)
	ctx := context.TODO()
	...
	dot := string(img)
	// Both flags on: parallel group should still be created
	assert.Contains(t, dot, `PARALLEL (2 JOBS)`)
	// Intermediate resource nodes should not appear (no cds shapes for artifact.output)
	assert.NotContains(t, dot, `"build-artifact.output-out"`)
	// Entry-point resource should be kept
	assert.Contains(t, dot, `"cron.timer"`)
	// Direct edge from build to group node
	assert.Contains(t, dot, `"build"`)
}

func TestGetPipelineImage_GroupParallel_EdgeRemapping(t *testing.T) {
	ctrl := gomock.NewController(t)
	s := newService(ctrl)
	ctx := context.TODO()
	...
	pp := &pipeline.Pipeline{
		...
		Jobs: []job.Job{
			...
			},
			{
				Name: "release",
				Plan: []job.PlanStep{
					{Type: job.StepTypeGet, Get: &job.GetStep{Type: "cron", Name: "timer", Trigger: true, Passed: []string{"lint", "vet"}}},
				},
			},
		},
		Resources: []resource.Resource{
	...
	dot := string(img)
	// lint and vet are grouped; edges from build should go to the group node
	assert.Contains(t, dot, `PARALLEL (2 JOBS)`)
